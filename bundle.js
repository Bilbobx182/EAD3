...
window.onload = function () {
    ...
    var ctx = clockCanvasElement.getContext("2d");
    var canvasWidth = window.innerWidth / 2;
    var canvasHeight = window.innerHeight / 2 + window.innerHeight / 3;
    ctx.canvas.width = canvasWidth;
    ctx.canvas.height = canvasHeight;

    var center_x = canvasWidth / 2;
    var center_y = canvasHeight - canvasHeight / 2;
    var radius = 350;
    var loopCount = 0;
    var digitPrinter = 0;
    var timeStampsLocationsX = [];
    var timeStampsLocationsY = [];
    var timeStampsLabel = [];
    ...
    function drawPoint(angle, distance, label) {
        ...
        if (label !== ".") {
            timeStampsLabel.push(label);
            timeStampsLocationsX.push(center_x + (radius - 25) * Math.cos(-angle * Math.PI / 180) * distance);
            timeStampsLocationsY.push(center_y + (radius - 25) * Math.sin(angle * Math.PI / 180) * distance);
        }

        ctx.fillText(label, x, y);
    }

    function drawHands(numberOnClock, strength) {
        ...
        ctx.lineTo(timeStampsLocationsX[numberOnClock], timeStampsLocationsY[numberOnClock]);
        ctx.stroke();
    }
    ...
    var location = 0;
    for (loopCount = -90; loopCount < 270; loopCount += 6) {
        drawPoint(loopCount, 1.05, location);
        location++;
    }

    // Testing the inner circle
    for (loopCount = -90; loopCount < 270; loopCount += 4) {
        var item = 0;
        ctx.fillText('.', timeStampsLocationsX[digitPrinter], timeStampsLocationsY[digitPrinter]);
        digitPrinter++;
    }
